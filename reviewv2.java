public class review {
    ...
    public class Baby { // class inside of a class (file_name is the overarching class, but to initiate Baby class, we need this within the file)
        static int babyID = 0; // now, if I change the babyID for alfred, it'll also change it for linda
        String name;
        boolean Female;
        double weight = 4.0;
        int numPoops = 0;
        public Baby (String babyName, boolean Fem) { // this is a constructor; basically just tells us how to make it/what to include when making a new baby
            ...
            Female = Fem; // the construc doesn't return anything (not even a null) so we don't put anything there
                    }
